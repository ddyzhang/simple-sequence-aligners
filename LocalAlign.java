...
public class LocalAlign {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < fastas.size(); i++) {
			...
			for (int k = 1; k <= reference.length(); k++) {
				for (int j = 1; j <= f.sequence.length(); j++) {
					m[k][j] = Math.max(0, getScore(reference.charAt(k-1), f.sequence.charAt(j-1)) + Math.max(m[k - 1][j - 1], Math.max(gx[k - 1][j - 1], gy[k - 1][j - 1])));
					gx[k][j] = Math.max(m[k - 1][j] + d, Math.max(gx[k - 1][j] + e, gy[k - 1][j] + d));
					gy[k][j] = Math.max(m[k][j - 1] + d, Math.max(gx[k][j - 1] + d, gy[k][j - 1] + e));
				}
			}

			// Set score to the max node value in the entire matrix
			int max = 0;
			...
			// Begin traceback
			traceBack(f, reference, m, gx, gy, maxReferenceIndex, maxSequenceIndex, 0);
		}
		...
		for (int i = 0; i < 3; i++) {
			...
			System.out.println("Starting Index in Query: " + f.referenceIndex);
			System.out.println("Starting Index in DB Sequence: " + f.fastaIndex);
			System.out.println("Alignment Length: " + f.y.length());
			System.out.println(f.y.substring(0, 60));
			System.out.println(f.x.substring(0, 60));	
			System.out.println("");		
		}
		
	}

	public static void traceBack(FastaSeq f, String reference, int[][] m, int[][] gx, int[][] gy, int k, int j, int state) {
		if (state == 0) {
			if (m[k][j] == 0) {
				f.fastaIndex = j + 1;
				f.referenceIndex = k + 1;
				return;
			} 
			f.x = reference.charAt(k-1) + f.x;
			f.y = f.sequence.charAt(j-1) + f.y;
			int mVal = getScore(reference.charAt(k-1), f.sequence.charAt(j-1)) + m[k - 1][j - 1];
			int gxVal = getScore(reference.charAt(k-1), f.sequence.charAt(j-1)) + gx[k - 1][j - 1];
			int gyVal = getScore(reference.charAt(k-1), f.sequence.charAt(j-1)) + gy[k - 1][j - 1];
			if (mVal >= gxVal && mVal >= gyVal) {
				traceBack(f, reference, m, gx, gy, k-1, j-1, 0);
			} else if (gxVal >= mVal && gxVal >= gyVal) {
				traceBack(f, reference, m, gx, gy, k-1, j-1, 1);
			} else {
				traceBack(f, reference, m, gx, gy, k-1, j-1, 2);
			}
		} else if (state == 1) {
			if (gx[k][j] == 0) {
				f.fastaIndex = j;
			...
			f.y = "-" + f.y;
			int mVal = m[k - 1][j] + d;
			int gxVal = gx[k - 1][j] + e;
			int gyVal = gy[k - 1][j] + d;
			if (mVal >= gxVal && mVal >= gyVal) {
				traceBack(f, reference, m, gx, gy, k-1, j, 0);
			} else if (gxVal >= mVal && gxVal >= gyVal) {
				traceBack(f, reference, m, gx, gy, k-1, j, 1);
			} else {
				traceBack(f, reference, m, gx, gy, k-1, j, 2);
			}
		} else {
			if (gy[k][j] == 0) {
				f.fastaIndex = j + 1;
			...
			f.y = f.sequence.charAt(j-1) + f.y;
			int mVal = m[k][j - 1] + d;
			int gxVal = gx[k][j - 1] + d;
			int gyVal = gy[k][j - 1] + e;
			if (mVal >= gxVal && mVal >= gyVal) {
				traceBack(f, reference, m, gx, gy, k, j-1, 0);
			} else if (gxVal >= mVal && gxVal >= gyVal) {
				traceBack(f, reference, m, gx, gy, k, j-1, 1);
			} else {
				traceBack(f, reference, m, gx, gy, k, j-1, 2);
			}
		}
	}
